        ...
        <section data-testid="section-future-events">
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1">
              <Calendar className="w-5 h-5 text-[#c94040]" />
          ...
          <div className="grid gap-4">
            {futureEvents.map((event) => (
              ...
              >
                <span className="text-black font-medium group-hover:text-[#c94040] transition-colors" data-testid={`text-event-${event.id}`}>
                  {event.title}
                </span>
                <ArrowRight className="w-4 h-4 text-[#c94040] opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
              </Link>
            ))}
          </div>
        </section>
          ...
          <div className="grid gap-4">
            {pastEvents.map((event) => (
              <Link
                ...
                data-testid={`link-event-${event.id}`}
              >
                <span className="text-black/80 font-medium group-hover:text-[#f08080] transition-colors" data-testid={`text-event-${event.id}`}>
                  {event.title}
                </span>
                <ArrowRight className="w-4 h-4 text-[#c94040] opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
              </Link>
